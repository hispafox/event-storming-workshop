...
function StickyNote({ type, text, isHighlighted, dimmed }) {
...
function FlowLane({ flow, activeType }) {
...
function CatalogPanel({ type, onClose }) {
  const config = ELEMENT_TYPES[type];
...
function MsTag({ items, color, icon }) {
...
function MicroservicesPanel({ onClose }) {
  return (
...
export default function EventStormingBoard() {
  ...
  const toggle = (type) => { setShowMs(false); setActiveType((prev) => (prev === type ? null : type)); };

  return (
    <div style={{ background: COLORS.bg, minHeight: "100vh", padding: "20px 16px", fontFamily: "'Segoe UI', system-ui, sans-serif", color: COLORS.text }}>
